...
class TCPNetwork : IPNetwork {
  public:
     ...
     * @param queue_size is how many connections requests can be queued
     */
    TCPNetwork(short port, int queue_size);
  ...
  private:
    int socket_fd = -1;
    std::map<IPAddress, int, IPAddressComparer> connections;
};
