...
class IPAddress : public sockaddr_in {
  public:
     ...
     */
    IPAddress(const char *host, short port) { resolve(host, port); }
    ...
    bool operator==(const IPAddress &other) const {
        return ((other.sin_port == sin_port) && (other.sin_addr.s_addr == sin_addr.s_addr));
    }
};

/**
 * @brief Compares two addresses with each other to create an ordering.
 * Used for std::map
 */
struct IPAddressComparer {
    bool operator()(const IPAddress &left, const IPAddress &right) const {
        return std::make_tuple(left.getAddr(), left.getPort()) < std::make_tuple(right.getAddr(), right.getPort());
    }
};
